...
utility_path = '/opt/chrome/tfinulx'

def wait_lock():
    timeout = 0
    locked = False
    while timeout < 30:
        if os.path.exists('/opt/chrome/odoo.lock'):
            time.sleep(2)
            timeout += 2
        else:
            lock = open('/opt/chrome/odoo.lock','w')
            lock.write('locked')
            lock.close()
            locked = True
            break
...
def clear_lock():
    os.unlink('/opt/chrome/odoo.lock')
...
def make_status(status):
    ...
    report.close()
    proc = Popen([utility_path, 'UploadStatusCmd', status, report.name], stdout=PIPE, stderr=PIPE)
...
def make_ticket(lines):
    #check_printer()
    log = open('/opt/chrome/odoo_exec.log', 'w')
    report = NamedTemporaryFile( suffix=".tmp", prefix="tmp__odoo_exec__", delete=False )
    for line in lines:
    ...
    log.close()
    report.close()
    proc = Popen([utility_path, 'SendFileCmd', report.name], stdout=PIPE, stderr=PIPE)
    print_out, check_err = proc.communicate()
...
def make_report(report_type):
    check_printer()
    proc = Popen([utility_path, 'SendCmd', report_type], stdout=PIPE, stderr=PIPE)
